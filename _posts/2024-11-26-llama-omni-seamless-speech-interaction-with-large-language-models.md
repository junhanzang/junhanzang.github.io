...
본 섹션에서는 LLaMA-Omni의 모델 아키텍처를 소개한다. 그림 2에서 볼 수 있듯이, LLaMA-Omni는 음성 인코더, 음성 어댑터, LLM, 음성 디코더로 구성되어 있다. 사용자의 음성 지시, 텍스트 응답, 음성 응답을 각각 *X<sub>S</sub>*, *Y<sub>T</sub>*, *Y<sub>S</sub>*로 표기한다.

### 2.1 음성 인코더

우리는 Whisper-large-v3 ([Radford et al., 2023](https://huggingface.co/openai/whisper-large-v3))의 인코더를 음성 인코더 *ℰ*로 사용한다. Whisper는 다량의 오디오 데이터를 학습한 범용 음성 인식 모델로, 음성으로부터 의미 있는 표현을 추출할 수 있는 능력을 가지고 있다. 특히, 사용자의 음성 지시 *X<sub>S</sub>*에 대해 인코딩된 음성 표현은 *𝐇 = ℰ(X<sub>S</sub>)*로 주어지며, 여기서 *𝐇 = [𝐡<sub>1</sub>, …, 𝐡<sub>N</sub>]*는 길이 *N*의 음성 표현 시퀀스를 나타낸다. 우리는 전체 학습 과정 동안 음성 인코더의 파라미터를 고정시킨다.
...
```
𝐇′ = [𝐡′<sub>1</sub>, …, 𝐡′<sub>⌊N/k⌋</sub>],
```

여기서

```
𝐡<sub>i</sub>′ = [𝐡<sub>k×(i−1)+1</sub> ⊕ 𝐡<sub>k×(i−1)+2</sub> ⊕ ⋯ ⊕ 𝐡<sub>k×i</sub>].
```
