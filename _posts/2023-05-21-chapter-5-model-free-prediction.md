...
![](/assets/images/posts/79/img.png)
...
목표는 정책 ?에 따른 에피소드 경험을 통해 ??(?)를 학습하는 것이다. 에피소드 경험은 상태 ?1, 행동 ?1, 보상 ?2, ..., 상태 ??을 ?에 따라 무작위로 선택하여 얻는 것이다.  
리턴은 총 감가된 보상을 나타내며, ??+1, ??+2, ..., ??을 ?로 감가하여 계산한다. ?는 할인율로, 미래의 보상에 대한 중요도를 조절한다.  
가치 함수는 상태 ?에서의 기대 리턴을 나타내며, 에피소드를 진행하면서 각 상태에서의 리턴을 측정하고 평균을 구한다. 이를 통해 가치 함수를 추청한다  
...
First-Visit Monte-Carlo Policy Evaluation은 특정 상태 ?를 평가하는 방법이다. 이 방법은 에피소드 동안 상태 ?가 처음으로 방문된 시점인 ?를 찾습니다.   
각 상태는 해당 상태를 방문한 횟수를 나타내는 카운터 ?(?)를 가지고 있다. 따라서 ??를 1 증가시킨다. 동시에 상태 ?에서 발생한 총 리턴을 나타내는 S?에 현재 리턴을 더 한다.   
가치 함수는 해당 상태의 평균 리턴으로 추정된다. 따라서 ?(?)는 S(?)를 ?(?)로 나눈 값으로 계산된다. 이를 통해 가치 함수를 추정할 수 있다.   
First-Visit Monte-Carlo Policy Evaluation은 상태 ?가 에피소드 동안 처음 방문된 시점인 ?를 찾고, 이를 기반으로 해당 상태의 가치를 추정한다. 큰 수의 법칙에 따라 ?(?)가 무한히 커질수록 ?(?)는 ??(?)로 수렴하게 된다.   
...
"Mars Rover" 예제에서는 우리가 상태 전이 확률 ?(?′|?,?)과 같은 모델에 대한 정보를 알지 못한다. 정책은 모든 상태에서 "왼쪽으로 이동"만 하는 것이며, 보상은 이동할 때마다 -1이 주어지고, S1에 도착하면 +1이 주어진다. 할인율은 ?=0.5로 설정된다.

![](/assets/images/posts/79/img_1.png)

First-Visit MC의 경우, 샘플 에피소드는 다음과 같다:   
• 에피소드 1: S2, -1, S3, -1, S2, +1, S1   
• 에피소드 2: S2, +1, S1

![](/assets/images/posts/79/img_2.png)

Every-Visit MC의 경우, 샘플 에피소드는 다음과 같다:   
• 에피소드 1: S2, -1, S3, -1, S2, +1, S1   
• 에피소드 2: S2, +1, S1

![](/assets/images/posts/79/img_3.png)
...
Incremental Monte-Carlo Update는 에피소드가 종료된 후 ?1,?1,?2,…,??~?와 같은 에피소드를 기반으로 가치 함수를 점진적으로 업데이트하는 방법이다.   
각 상태 ??에 대해 해당 상태의 리턴 ??을 이용하여 가치 함수를 업데이트한다. ?(??)는 해당 상태를 방문한 횟수를 나타내는 카운터로, ?(??)를 1 증가시킵니다. 그리고 ?(??)를 현재의 가치 함수 값에서 다음과 같이 업데이트합니다:

?(??)←?(??)+(1/?(??))(??−?(??)).   
또한, 비정상적인(non-stationary) 문제의 경우, 진행되는 평균을 추적하는 것이 유용할 수 있다. ?(??)를 다음과 같이 업데이트하여 이전 데이터를 잊어버리지 않고 가치 함수를 추정한다: ?(??)←?(??)+?(??−?(??)).   
여기서 ?는 학습률을 나타내며, 1/?(?)보다 큰 값을 설정하면 이전 데이터를 빠르게 잊어버릴 수 있다. 이를 통해 최신 데이터에 더 많은 중요성을 부여하여 가치 함수를 업데이트할 수 있다.   
...
Monte Carlo와 Temporal Difference는 모두 정책 ?에 따른 에피소드 경험을 통해 가치 함수 ??(?)를 학습하는 방법이다.  
Monte Carlo 방법에서는 인크리멘털 모델로 업데이트하는 방식을 사용한다. 즉, ?(??)를 다음과 같이 업데이트한다: ???←???+?(??−???).

여기서 ?는 학습률을 의미한다. Monte Carlo 방법은 에피소드가 끝나야 전체 리턴을 계산하고 가치 함수를 업데이트한다.   
  
반면 Temporal Difference 방법은 TD 학습 알고리즘을 사용한다. 가치 함수를 추정하기 위해 ?(?+1)+??(??+1)을 추정값으로 설정하여 ?(??)를 다음과 같이 업데이트한다: ?(??)←?(??)+?(?(?+1)+??(??+1)−?(??)). 여기서 ?(?+1)+??(??+1)을 TD 타겟(TD target)이라고 부릅니다. 또한, ?(?+1)+??(??+1)−?(??)는 TD 오차(TD error)라고 부릅니다.   
...
![](/assets/images/posts/79/img_4.png)

Driving Home Example

![](/assets/images/posts/79/img_5.png)

![](/assets/images/posts/79/img_6.png)
...
![](/assets/images/posts/79/img_7.png)

Return ??=?(?+1)+??(?+2)+...은 ??(??)의 무편향(unbiased) 추정치다. 즉, 이는 ??(??)를 정확하게 추정하는 경향이 있다.   
  
실제 TD 타겟은 ??(??)의 무편향 추정치다. 하지만 보통 TD 타겟은 ??(??)의 불완전한(biased) 추정치다. 즉, TD 타겟은 정확한 추정치가 아닐 수 있다.   
...
Temporal Difference 방법은 분산이 낮지만 약간의 편향이 있을 수 있다. 일반적으로 Monte Carlo 방법보다 효율적이다. Temporal Difference 방법은 ???에 수렴하지만 함수 근사화와 함께 사용할 때 항상 수렴하지는 않을 수 있다. 초기값에 대해 민감도가 더 높다.   
...
Batch MC and TD는 유한한 데이터셋에 대한 배치(오프라인) 솔루션이다. 주어진 K개의 에피소드 집합이 있을 때, 반복적으로 ?∈[1,?] 범위에서 에피소드를 샘플링하고, 해당 샘플링된 에피소드에 대해 MC 또는 TD를 적용한다.   
...
AB 예제에서는 두 개의 상태 ?와 ?가 있으며 할인율(?)은 1로 설정됩니다. 주어진 8개의 에피소드 경험은 다음과 같다:   
...
![](/assets/images/posts/79/img_8.png)

주어진 에피소드에서 ?와 ?의 가치 함수 ?(?)과 ?(?)을 추정해보겠습니다.   
  
먼저, MC(Monte Carlo) 방법을 사용하여 가치 함수를 추정하면 다음과 같습니다:   
• ?(?) = (0 + 0) / 2 = 0   
• ?(?) = (1 + 1 + 1 + 0 + 1 + 1 + 1 + 1) / 8 = 0.875   
  
다음으로, TD(Temporal Difference) 방법을 사용하여 가치 함수를 추정하면 다음과 같습니다:   
• ?(?) = 0 + 1 = 1   
• ?(?) = 1 + 0.875 = 1.875   
  
따라서, 주어진 8개의 에피소드에서 ?의 가치 함수는 0 또는 1로, ?의 가치 함수는 0.875 또는 1.875로 추정됩니다.

추가적으로 사용시 MC(Monte Carlo)와 TD(Temporal Difference) 방법의 차이점을 알아보자.  
  
가장 간단한 TD 방법에서는 (?,?,?,?′)를 사용하여 ?(?)를 업데이트합니다. 즉, ?(?)를 업데이트하는 데에는 ?(1)의 연산이 필요합니다. 에피소드의 길이가 ?인 경우에도 ?(?)의 연산이 필요합니다.   
  
반면 MC 방법에서는 에피소드가 끝날 때까지 기다려야하며, 이때도 업데이트에는 ?(?)의 연산이 필요합니다.   
  
TD 방법은 Markov 구조를 활용합니다. 즉, (?,?,?,?′)의 정보를 활용하여 가치 함수를 업데이트합니다. Markov 도메인에서 이러한 구조를 활용하는 것이 도움이 됩니다.   
  
반면 MC 방법은 Markov 속성을 활용하지 않습니다. 따라서 비-Markov 환경에서는 보통 MC 방법이 더 효과적입니다.   
  
결론적으로, TD 방법은 ?(1)의 연산으로 가치 함수를 업데이트하며 Markov 구조를 활용합니다. 반면 MC 방법은 에피소드의 종료까지 기다려야하며 Markov 속성을 활용하지 않습니다. 따라서 비-Markov 환경에서는 MC 방법이 일반적으로 더 효과적입니다.

**DP vs. MC vs. TD**

![](/assets/images/posts/79/img_9.png)
...
![](/assets/images/posts/79/img_10.png)

**?-Step Prediction**

?-Step Prediction은 TD 타겟이 미래로 ?스텝을 바라보도록 하는 것을 의미합니다.   
  
일반적으로 TD(Temporal Difference) 학습에서는 TD 타겟을 현재 보상과 다음 상태의 가치 함수를 이용하여 업데이트합니다. 하지만 ?-Step Prediction에서는 TD 타겟이 더 멀리 미래의 보상과 상태의 가치 함수를 고려하여 업데이트합니다. 즉, ?스텝만큼 미래로 전진하여 TD 타겟을 계산합니다.   
  
예를 들어, ?=2로 설정한 경우, TD 타겟은 현재 보상과 현재 상태에서 2스텝 후의 보상과 상태의 가치 함수를 고려하여 계산됩니다. 이를 통해 미래의 정보를 좀 더 포함한 업데이트를 수행할 수 있습니다.   
  
?-Step Prediction은 ?의 값을 조정함으로써 현재와 멀리 미래의 정보를 얼마나 고려할지 조절할 수 있습니다. 작은 ?값은 즉각적인 보상에 더 중점을 두며, 큰 ?값은 장기적인 보상을 고려합니다.

![](/assets/images/posts/79/img_11.png)

?-Step Prediction

**?-Step Return**

?-Step Return은 ?=1,2,..∞에 대한 다음과 같은 n-스텝 리턴을 고려합니다:

![](/assets/images/posts/79/img_12.png)
...
![](/assets/images/posts/79/img_13.png)
...
![](/assets/images/posts/79/img_14.png)

?-Step Return은 ?의 값을 조정함으로써 현재와 멀리 미래의 정보를 얼마나 고려할지 조절할 수 있습니다. 작은 ?값은 즉각적인 보상에 더 중점을 두며, 큰 ?값은 장기적인 보상을 고려합니다.

![](/assets/images/posts/79/img_15.png)

?-Step at Random Walk Example

**Averaging ?-Step Returns**

우리는 서로 다른 ?에 대한 ?-스텝 리턴들을 평균화할 수 있습니다. 예를 들어, 2-스텝과 4-스텝 리턴을 평균화할 수 있습니다. 이를 통해 두 가지 다른 시간 단계의 정보를 결합할 수 있습니다. 즉, 2-스텝과 4-스텝 리턴의 평균값을 계산하여 정보를 결합합니다.   
...
그러나 ?-스텝 리턴을 평균화하는 것은 두 개의 시간 단계를 결합하는 방법으로, 상대적으로 계산량이 적은 방법입니다. 따라서 보통은 일부 시간 단계에 대한 평균 리턴을 사용하여 정보를 결합합니다.

?-return

?-리턴은 TD(?) 방법의 한 종류로, ? 값에 따라 ?-스텝 업데이트를 평균화하는 방식입니다. ? 값을 [0, 1] 범위에서 선택하며, 각각의 업데이트는 ??−1에 비례하여 가중치를 가지게 됩니다.   
  
?-리턴은 다음과 같이 계산됩니다:

![](/assets/images/posts/79/img_16.png)

여기서, ??는 ?-스텝 리턴이고, ??+1은 ?+1 시점에서의 가치 추정치입니다.   
  
?-리턴을 이용한 백업은 다음과 같이 수행됩니다:

![](/assets/images/posts/79/img_17.png)

이를 통해 현재 상태 ?에서의 가치 함수 추정치를 ?-리턴을 활용하여 업데이트합니다. ?는 학습률을 의미하며, 업데이트의 크기를 조절하는 역할을 합니다.   
  
?-리턴은 ? 값에 따라 즉각적인 리턴과 장기적인 리턴을 조절할 수 있습니다. ? 값을 0에 가까워지게 설정하면 즉각적인 리턴에 가중치를 두고, 1에 가까워지게 설정하면 장기적인 리턴에 가중치를 둘 수 있습니다.

![](/assets/images/posts/79/img_18.png)

?-리턴

Relation to TD and MC

?-리턴에서 ? 값을 조절함에 따라 다음과 같은 관계가 있습니다:   
  
만약 ?=0이라면, ?-리턴은 한 단계 TD(0)와 동일해집니다. 즉, 현재 상태에서 바로 다음 상태로의 리턴을 계산하여 업데이트합니다. 이는 한 단계 TD(0) 방법과 같은 방식으로 동작합니다.   
  
반대로, ?=1이라면, ?-리턴은 MC와 동일해집니다. 즉, 전체 에피소드를 통해 리턴을 계산하고 업데이트합니다. MC는 모든 에피소드가 종료될 때까지 기다려야하고, 에피소드가 종료된 후에 업데이트를 수행합니다.

![](/assets/images/posts/79/img_19.png)

따라서 ? 값을 조절함으로써 ?-리턴은 TD와 MC 사이의 중간 방법으로 작동합니다. ? 값을 0에 가깝게 설정하면 한 단계 TD(0)에 가까워지고, 1에 가깝게 설정하면 MC에 가까워집니다.

Forward-view TD(?)

![](/assets/images/posts/79/img_20.png)

Forward-view TD(?)

Forward-view TD(?)는 ?-리턴을 기준으로 가치 함수를 업데이트하는 방법입니다. Forward-view는 미래를 바라보며 ???를 계산합니다. 따라서, MC와 마찬가지로 완전한 에피소드에서만 계산할 수 있습니다.   
  
Forward-view TD(?)는 다음과 같은 방식으로 가치 함수를 업데이트합니다. ?-리턴인 ???를 이용하여 가치 함수를 계산한 후, 현재 가치 함수와의 차이를 학습률을 곱하여 업데이트합니다.   
  
??? = ??? + ?(??? - ???)   
  
Forward-view TD(?)는 MC와 유사한 방식으로 동작하지만, ?-리턴을 사용하여 가치 함수를 업데이트한다는 점에서 차이가 있습니다. 따라서, 완전한 에피소드가 필요하며, 에피소드가 종료된 후에 업데이트를 수행해야 합니다.

![](/assets/images/posts/79/img_21.png)

Forward-View TD(?) on Random Walk
