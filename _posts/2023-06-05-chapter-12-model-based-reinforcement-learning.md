...
![](/assets/images/posts/87/img.png)
...
![](/assets/images/posts/87/img_1.png)
...
![](/assets/images/posts/87/img_2.png)
...
![](/assets/images/posts/87/img_3.png)

그렇다면 Model은 무엇일까요?

Model은 상태 공간 ?, 행동 공간 ?, 상태 전이 확률 ?, 보상 함수 ? 등으로 구성된 MDP(Markov Decision Process) ?의 표현입니다. ?은 파라미터 ?로 매개화되며, 모델 ??은 상태 전이 확률 ??≈?와 보상 함수 ??≈?를 나타냅니다.

그렇다면 이 model을 어떻게 학습시킬까?

Model Learning은 경험 데이터 ?1,?1,?2,...,??로부터 모델 ??을 추정하는 것을 목표로 합니다. 이는 지도 학습 문제로 볼 수 있으며, ?,?,→?은 회귀 문제로, ?,?,→?′는 밀도 추정 문제로 간주됩니다. 손실 함수(예: 평균 제곱 오차, KL 다이버전스 등)를 선택하고, 경험 데이터에 대한 경험적 손실을 최소화하는 파라미터 ?를 찾습니다.

이를 합쳐 Table Lookup Model라하며 Table Lookup Model은 명시적인 MDP, ?,? ?으로 모델이 구성된 경우를 의미합니다. 각 상태-행동 쌍의 방문 횟수 ?(?,?)를 계산합니다.

![](/assets/images/posts/87/img_4.png)

또는 각 시간 단계에서 경험 튜플 ??,??,??+1,??+1과 같은 튜플을 기록합니다. 모델을 샘플링하기 위해 ?,?,∙,∙과 일치하는 튜플을 무작위로 선택합니다.

이를 **AB Example**에 대해 적용해보자.  
AB Example은 이전 chapter에서 사용했으니 설명은 넘어가겠습니다.

![](/assets/images/posts/87/img_5.png)

AB Example

**Planning with a Model**

주어진 모델 ??=??,??을 활용하여 MDP를 해결하는 것이 목표입니다. 이를 위해 여러 가지 계획 알고리즘 중 선호하는 방법을 사용합니다. 예를 들어, Policy Iteration이나 Value Iteration과 같은 알고리즘을 활용하여 최적의 정책을 계획합니다. 모델을 사용하여 상태 전이 확률과 보상 함수를 이용하여 계획을 수행합니다.
...
![](/assets/images/posts/87/img_6.png)

**Planning with an Inaccurate Model**

주어진 부정확한 모델 ??,??≠?,?을 가지고 있는 경우, 모델 기반 RL의 성능은 근사적인 MDP ?,?,??,??에 대한 최적 정책에 제한됩니다. 이 경우 모델이 부정확할 때, 계획 과정은 부적합한 정책을 계산하게 됩니다. 모델 기반 RL은 추정된 모델만큼 정확하다고 할 수 있습니다.   
...
![](/assets/images/posts/87/img_7.png)
...
![](/assets/images/posts/87/img_8.png)
...
![](/assets/images/posts/87/img_9.png)
...
![](/assets/images/posts/87/img_10.png)
...
![](/assets/images/posts/87/img_11.png)
...
 Forward Search 알고리즘은 전방 탐색을 통해 최적의 행동을 선택합니다. 이를 위해 현재 상태 ??를 루트로 하는 탐색 트리를 구축하며, MDP의 모델을 사용하여 미래를 예측합니다. 전체 MDP를 해결할 필요 없이 현재 시점에서 시작하는 하위 MDP(sub MDP)만 해결하면 됩니다.   
...
![](/assets/images/posts/87/img_12.png)
...
![](/assets/images/posts/87/img_13.png)

Simulation Based Search은 전방 탐색을 기반으로 하며, 모델을 사용하여 경험의 에피소드를 시뮬레이션하고, 이를 Model-free RL에 적용함으로써 최적의 결정을 내릴 수 있는 방법입니다.

![](/assets/images/posts/87/img_14.png)

Simulation&nbsp;Based&nbsp;Search

**Simple Monte-Carlo Search**

Simple Monte Carlo Search은 주어진 모델 ??와 시뮬레이션 정책 ?를 기반으로 합니다. 각 행동 ? ∈ ?에 대해 다음과 같은 과정을 수행합니다.   
  
먼저, 현재 상태 ??에서 시작하여 ?개의 에피소드를 시뮬레이션합니다. 실제 상태에서부터 모델을 사용하여 ?개의 에피소드를 생성합니다.

![](/assets/images/posts/87/img_15.png)

그 후, 각 에피소드의 반환값의 평균을 계산하여 각 행동의 가치를 평가합니다. 이것을 Monte Carlo 평가라고 합니다. 가치 평가를 통해 현재 상태에서 각 행동의 예상 이익을 추정할 수 있습니다.

![](/assets/images/posts/87/img_16.png)
...
![](/assets/images/posts/87/img_17.png)

Simple Monte Carlo Search은 주어진 모델과 시뮬레이션 정책을 사용하여 가치 평가를 수행하고, 최적의 행동을 선택합니다. 이를 통해 모델 기반 RL에서 효과적인 학습과 결정을 가능하게 합니다.

**Monte-Carlo Tree Search (Evaluation)**

Monte-Carlo Tree Search는 주어진 모델 ??을 활용하여 트리를 구축하고, 각 상태와 행동의 가치를 평가하는 방법입니다. 이를 통해 최적의 행동을 선택합니다. 이제 각 단계에 대해 자세히 알아보겠습니다.   
  
먼저, 현재 상태 ??에서 시작하여 ?개의 에피소드를 시뮬레이션합니다. 현재 상태에서부터 모델을 사용하여 ?개의 에피소드를 생성합니다. 그리고 방문한 상태와 행동을 포함하는 탐색 트리를 구축합니다.

![](/assets/images/posts/87/img_18.png)
...
![](/assets/images/posts/87/img_19.png)
...
![](/assets/images/posts/87/img_20.png)

Monte-Carlo Tree Search는 주어진 모델을 활용하여 트리를 구축하고 가치 평가를 통해 최적의 행동을 선택합니다. 이를 통해 모델 기반 RL에서 효과적인 학습과 결정을 가능하게 합니다.

**Monte-Carlo Tree Search (Simulation)**

Monte-Carlo Tree Search에서는 시뮬레이션 정책 ?이 개선됩니다. 각 시뮬레이션은 두 단계로 구성됩니다. 이제 각 단계에 대해 자세히 알아보겠습니다.   
  
먼저, 시뮬레이션에서는 트리 정책(Tree policy)와 기본/롤아웃 정책(Default/Rollout policy)의 두 가지 단계로 진행됩니다. 트리 정책은 ?(?,?)를 최대화하는 행동을 선택하는 단계입니다. 기본/롤아웃 정책은 무작위로 행동을 선택하거나 다른 정책을 사용하는 단계입니다.   
  
각 시뮬레이션마다 다음을 반복합니다. 먼저, Monte Carlo 평가를 통해 상태-행동 가치를 평가합니다. 이를 통해 ?(?,?)를 추정할 수 있습니다. 그런 다음 트리 정책을 개선합니다. 예를 들어, ?-greedy(Q)와 같은 방식으로 트리 정책을 개선할 수 있습니다.   
